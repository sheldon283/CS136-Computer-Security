...
$IPTABLES -i $ETH -A INPUT -s $SERVER -j DROP
...
$IPTABLES -i $ETH -A INPUT -m state --state ESTABLISHED,RELATED -j ACCEPT
$IPTABLES -o $ETH -A OUTPUT -m state --state ESTABLISHED,RELATED -j ACCEPT
...
$IPTABLES -i $ETH -A INPUT -m state --state NEW -p tcp --dport 22 -j ACCEPT
$IPTABLES -i $ETH -A INPUT -m state --state NEW -p tcp --dport 80 -j ACCEPT
$IPTABLES -i $ETH -A INPUT -m state --state NEW -p tcp -s $CLIENT --dport 3306 -j ACCEPT

# 2. allow new outbound tcp traffic to remote systems running OpenSSH,
# Apache, and SMTP servers (on their standard ports).

$IPTABLES -o $ETH -A OUTPUT -m state --state NEW -p tcp --dport 22 -j ACCEPT
$IPTABLES -o $ETH -A OUTPUT -m state --state NEW -p tcp --dport 80 -j ACCEPT
$IPTABLES -o $ETH -A OUTPUT -m state --state NEW -p tcp --dport 3306 -j ACCEPT
...
$IPTABLES -i $ETH -A INPUT -m state --state NEW -p udp -s $CLIENT -d $SERVER --dport 10000:10005 -j ACCEPT
$IPTABLES -o $ETH -A OUTPUT -m state --state NEW -p udp -s $SERVER -d $CLIENT --dport 10006:10010 -j ACCEPT

# 4. allow the server to send and respond to ICMP pings.

$IPTABLES -i $ETH -A INPUT -m state --state NEW -p icmp --icmp-type echo-request -j ACCEPT
$IPTABLES -o $ETH -A OUTPUT -m state --state NEW -p icmp --icmp-type echo-reply -j ACCEPT
$IPTABLES -o $ETH -A OUTPUT -m state --state NEW -p icmp --icmp-type echo-request -j ACCEPT
$IPTABLES -i $ETH -A INPUT -m state --state NEW -p icmp --icmp-type echo-reply -j ACCEPT
